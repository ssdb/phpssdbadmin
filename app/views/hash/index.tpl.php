...
<div style="float: left;">
	<a class="btn btn-xs btn-primary" href="<?php echo _url('hash/hset')?>">
...
<tbody>
	...
	<tr>
		...
		<td><a href="<?php echo _url('hash/hscan', array('n'=>$k))?>"><?php echo htmlspecialchars($k)?></a></td>
		<td><?php echo $v?></td>
		<td>
			<a class="btn btn-xs btn-danger" href="<?php echo _url('hash/hclear', array('n'=>$k))?>" title="Remove">
...
function remove_selected(){
	...
	var url = <?php echo json_encode(_url('hash/hclear'))?> + '?' + $.param({n: ks});
	location.href = url;
}
</script>
...
<p class="pager">
	...
	<?php
		...
		$ks = array_keys($kvs);
		$start = $ks[0];
		$url = _url('hash', array('dir'=>'prev', 's'=>$start, 'e'=>'', 'size'=>$size));
	?>
	...
	<?php
		...
		$ks = array_keys($kvs);
		$start = $ks[count($ks)-1];
		$url = _url('hash', array('dir'=>'next', 's'=>$start, 'e'=>'', 'size'=>$size));
	?>
		<a class="btn btn-sm btn-primary" href="<?php echo $url?>">
			Next <i class="glyphicon glyphicon-chevron-right"></i>
		</a>
