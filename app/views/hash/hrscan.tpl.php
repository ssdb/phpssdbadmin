...
<tbody>
	...
	<tr>
		...
		<td>
			...
			<a class="btn btn-xs btn-danger" href="<?php echo _url('hash/hdel', array('n'=>$n, 'k'=>$k))?>" title="Remove">
...
<p class="pager">
	...
	<?php
		...
		$ks = array_keys($kvs);
		$start = $ks[0];
		$url = _url('hash/hrscan', array('dir'=>'prev', 'n'=>$n, 's'=>$start, 'e'=>'', 'size'=>$size));
	?>
	...
	<?php
		...
		$ks = array_keys($kvs);
		$start = $ks[count($ks)-1];
		$url = _url('hash/hrscan', array('dir'=>'next', 'n'=>$n, 's'=>$start, 'e'=>'', 'size'=>$size));
	?>
		<a class="btn btn-sm btn-primary" href="<?php echo $url?>">
			Next <i class="glyphicon glyphicon-chevron-right"></i>
		</a>
